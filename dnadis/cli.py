#!/usr/bin/env python3
"""Command-line interface for dnadis.

This module provides the main entry point for the dnadis tool,
...
from dnadis.output.fasta_output import write_classified_fastas, write_per_subgenome_chrs_fastas
...
from dnadis.output.plotting import run_assembly_report
from dnadis.models import AssemblyResult, ReferenceContext
...
def run_assembly(
    ...
    from dnadis.utils.distributed import LocalExecutor, LocalFuture, ResourceSpec
    if executor is None:
    ...
    if use_cluster:
        from dnadis.utils.resource_estimation import estimate_synteny_resources
    ...
    if use_cluster:
        from dnadis.utils.resource_estimation import estimate_blast_resources
    ...
    if chromosome_contigs:
        if use_cluster:
            from dnadis.utils.resource_estimation import estimate_debris_resources
    ...
    if not args.skip_telomeres:
        ...
        from dnadis.detection.telomere import detect_telomeres
    ...
    from dnadis.output.tsv_output import write_rearrangements_tsv
    ...
    if reads and not args.skip_depth:
        ...
        from dnadis.analysis.read_depth import calculate_depth_metrics
        from dnadis.models import DepthStats

        if use_cluster:
            from dnadis.utils.resource_estimation import estimate_depth_resources
    ...
    if args.scaffold:
        ...
        from dnadis.output.scaffolding import scaffold_chromosomes, write_agp, orientations_from_agp
        ...
        if scaffolded_seqs:
            from dnadis.utils.sequence_utils import write_fasta as _write_fasta
    ...
    from dnadis.output.comparison import build_assembly_result
...
def main():
    ...
    if '--dump-config' in sys.argv:
        from dnadis.utils.config import dump_config_template
    ...
    p = argparse.ArgumentParser(
        description="Genome assembly comprehension and curation tool for contig classification using synteny evidence (protein-anchored or nucleotide whole-genome alignment).",
    ...
    if args.config:
        from dnadis.utils.config import load_config, merge_config_with_args
        try:
    ...
    if not log_file:
        log_file = str(output_dir / "dnadis.log")

    from dnadis.utils.logging_config import setup_logging, get_logger
    ...
    if is_multi:
        from dnadis.utils.multi_assembly import resolve_assemblies
    ...
    if n_total > 1 and results:
        ...
        if not args.skip_plot:
            from dnadis.output.plotting import run_comparison_report
