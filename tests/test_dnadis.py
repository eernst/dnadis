...
import dnadis
from dnadis.alignment.chain_parsing import _filter_overlapping_hits_by_identity
from dnadis.utils.reference_utils import _REF_ID_PATTERNS
...
def test_normalize_ref_id_case_variations():
    ...
    assert dnadis.normalize_ref_id("chr1") == "chr1"
    assert dnadis.normalize_ref_id("chrC") == "chrC"

    # Uppercase Chr -> chr
    assert dnadis.normalize_ref_id("Chr1") == "chr1"
    assert dnadis.normalize_ref_id("ChrC") == "chrC"
    assert dnadis.normalize_ref_id("Chr10") == "chr10"

    # All uppercase CHR -> chr
    assert dnadis.normalize_ref_id("CHR1") == "chr1"
    assert dnadis.normalize_ref_id("CHRM") == "chrM"

    # Non-chr prefixes unchanged
    assert dnadis.normalize_ref_id("scaffold_1") == "scaffold_1"
    assert dnadis.normalize_ref_id("contig123") == "contig123"
    assert dnadis.normalize_ref_id("Mt") == "Mt"
...
def test_normalize_organelle_id_aliases():
    ...
    assert dnadis.normalize_organelle_id("CpDNA") == "chrC"
    assert dnadis.normalize_organelle_id("Chloroplast") == "chrC"
    assert dnadis.normalize_organelle_id("Mitochondrion") == "chrM"
    assert dnadis.normalize_organelle_id("mitochondria") == "chrM"

    # Non-organelles return None
    assert dnadis.normalize_organelle_id("chr1") is None
    assert dnadis.normalize_organelle_id("Chr5") is None
    assert dnadis.normalize_organelle_id("scaffold_1") is None


def test_set_ref_id_patterns_and_reset():
    """Test that custom patterns can be set and reset."""
    # Save original state
    original_patterns = dnadis.get_ref_id_patterns()

    try:
        # Set custom patterns
        custom = [re.compile(r"^(?P<chrom>scaffold\d+)(?P<sg>[XY])$")]
        dnadis.set_ref_id_patterns(custom)
        assert dnadis.get_ref_id_patterns() == custom

        # Test split_chrom_subgenome uses custom patterns
        assert dnadis.split_chrom_subgenome("scaffold1X") == ("scaffold1", "X")
        assert dnadis.split_chrom_subgenome("scaffold2Y") == ("scaffold2", "Y")

        # Non-matching IDs fall through
        assert dnadis.split_chrom_subgenome("chr1A") == ("chr1A", "NA")

        # Reset to defaults
        dnadis.set_ref_id_patterns(None)
        assert dnadis.get_ref_id_patterns() == _REF_ID_PATTERNS

        # Verify default patterns work again
        assert dnadis.split_chrom_subgenome("chr1A") == ("chr1", "A")
    finally:
...
def test_split_chrom_subgenome_with_explicit_patterns():
    ...
    assert dnadis.split_chrom_subgenome("chr1At", patterns=cotton_patterns) == ("chr1", "At")
    assert dnadis.split_chrom_subgenome("chr5Dt", patterns=cotton_patterns) == ("chr5", "Dt")
...
def test_split_chrom_subgenome_arabidopsis_conventions():
    ...
    assert dnadis.split_chrom_subgenome("chr1A") == ("chr1", "A")
    assert dnadis.split_chrom_subgenome("chr5B") == ("chr5", "B")
    assert dnadis.split_chrom_subgenome("chrC") == ("chrC", "NA")
    assert dnadis.split_chrom_subgenome("chrM") == ("chrM", "NA")
    assert dnadis.split_chrom_subgenome("scaffold_12") == ("scaffold_12", "NA")
...
def test_read_fasta_lengths_and_ref_lengths_tsv(tmp_path):
    ...
    lengths = dnadis.read_fasta_lengths(fasta_path)
    assert lengths["chr1"] == 10
    assert lengths["chr2"] == 6
    assert lengths["chrC"] == 4
    assert lengths["chrM"] == 5

    assert dnadis.get_min_nuclear_chrom_length(lengths) == 6

    tsv_path = tmp_path / "ref_lengths.tsv"
    dnadis.write_ref_lengths_tsv(tsv_path, fasta_path)
...
def test_parse_gff3_transcript_coords_araport11(tmp_path):
    ...
    tx2loc, tx2gene = dnadis.parse_gff3_transcript_coords(gff3_path)
...
def test_parse_blast_coverage(tmp_path):
    ...
    results = dnadis.parse_blast_coverage(blast_path, query_lengths)
...
def test_filter_overlapping_hits_by_identity():
    ...
    blocks = {
        ("contigA", "chr1A", "+"): [
            dnadis.Block(qs=100, qe=200, rs=1000, re=1100, matches=95, aln_len=100, mapq=60, strand="+", gene_id="geneA1"),
            dnadis.Block(qs=300, qe=400, rs=2000, re=2100, matches=90, aln_len=100, mapq=60, strand="+", gene_id="geneA2"),
        ],
        ("contigA", "chr1P", "+"): [
            # Overlaps with geneA1 at same position but lower identity
            dnadis.Block(qs=100, qe=200, rs=1000, re=1100, matches=85, aln_len=100, mapq=60, strand="+", gene_id="geneP1"),
            # Non-overlapping hit should be kept
            dnadis.Block(qs=500, qe=600, rs=3000, re=3100, matches=80, aln_len=100, mapq=60, strand="+", gene_id="geneP2"),
        ],
    }

    filtered, n_before, n_removed, removed_per_contig = _filter_overlapping_hits_by_identity(blocks)
...
def test_filter_overlapping_hits_higher_identity_wins():
    ...
    blocks = {
        ("contigA", "chr1A", "+"): [
            dnadis.Block(qs=100, qe=200, rs=1000, re=1100, matches=80, aln_len=100, mapq=60, strand="+", gene_id="geneA1"),
        ],
        ("contigA", "chr1P", "+"): [
            # Overlaps with geneA1 but has HIGHER identity - should win
            dnadis.Block(qs=100, qe=200, rs=1000, re=1100, matches=95, aln_len=100, mapq=60, strand="+", gene_id="geneP1"),
        ],
    }
...
def test_telomere_detection_plant_motif():
    ...
    from dnadis.detection.telomere import detect_telomeres_single, _count_telomere_repeats
...
def test_telomere_detection_at_contig_ends():
    ...
    from dnadis.detection.telomere import detect_telomeres_single
...
def test_classify_full_length_both_telomeres():
    ...
    from dnadis.classification.classifier import classify_full_length
...
def test_compute_largest_cluster_single_ref():
    ...
    from dnadis.classification.classifier import compute_largest_cluster_metrics_per_ref
...
def test_detect_rearrangement_candidates_no_offtarget():
    ...
    from dnadis.classification.classifier import detect_rearrangement_candidates
...
def test_compute_reference_gene_density():
    ...
    from dnadis.classification.classifier import compute_reference_gene_density
...
def test_builtin_scaffold_ordering():
    ...
    from dnadis.output.scaffolding import _builtin_scaffold
...
def test_agp_format():
    ...
    from dnadis.output.scaffolding import write_agp, _agp_component_line, _agp_gap_line
...
def test_compute_collinearity_score_perfect():
    ...
    from dnadis.alignment.chain_parsing import compute_collinearity_score
...
def _make_rdna_locus(contig, start, end, strand="+", identity=0.95,
    ...
    from dnadis.models import RdnaLocus
    return RdnaLocus(
        contig=contig,
        start=start,
        end=end,
...
def test_detect_arrays_basic():
    ...
    from dnadis.detection.rdna_consensus import _detect_arrays
...
def test_gmm_1d_em_single_component():
    ...
    from dnadis.classification.classifier import _gmm_1d_em
...
def test_gmm_1d_bic_selects_k1_for_unimodal():
    ...
    from dnadis.classification.classifier import _gmm_1d_bic
...
def test_resolve_reciprocal_translocation_basic():
    ...
    """
    from dnadis.cli import _resolve_reciprocal_translocations
