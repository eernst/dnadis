...
* [ ] **Reference chromosome filtering**: Filter reference sequences used for synteny assignment to exclude small scaffolds that can soak up spurious mappings and add noise. Precedence:
  ...
  2. **Naming convention detection**: If no list provided, test reference sequence names against widely used chromosome naming conventions (chr1, Chr1, chromosome_1, etc. — extend `is_nuclear_chromosome()`). Keep only sequences matching recognized chromosome patterns. I expect this to be the most commonly used code path.

  3. **Automatic gap detection**: If naming conventions don't match (e.g., scaffold_001, contig_123), sort reference sequences by length, detect the largest length gap, and retain only sequences above the gap. This handles references with a clear size discontinuity between chromosomes and unplaced scaffolds. There should be some sanity checks here, as this method won't work for smooth size distributions (highly fragmented assemblies like those produced by short-read pipelines, low coverage long-read assemblies, or genomes with smaller chromosomes assembled with poor heterozygosity collapsing).
